...
-- 1 'elem' [1, 2] -- True -----> testing if x is element of list xs
-- elem x xs == x 'elem' xs | x:[] ++ == [x] ++
-- if elem x xs then seen
-- else x:[] ++ seen ---- put the removed one into list
-- rest xs --- simply leave the rest
